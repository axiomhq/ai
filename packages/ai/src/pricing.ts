...
export class _SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED_Pricing {
  ...
  static calculateCost(inputTokens: number, outputTokens: number, model: string): number {
    const modelPricing =
      _SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED_Pricing.ModelPrices[model];
    ...
    const inputCost = (inputTokens / 1000) * modelPricing.INPUT_TOKEN_COST_PER_1K;
    const outputCost = (outputTokens / 1000) * modelPricing.OUTPUT_TOKEN_COST_PER_1K;
