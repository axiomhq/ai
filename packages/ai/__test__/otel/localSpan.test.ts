...
describe('LocalSpan', () => {
  ...
  it('should generate proper OpenTelemetry ID formats', () => {
    const span = new LocalSpan('test-span', 0);
    ...
    // IDs should be unique on each span creation
    const span2 = new LocalSpan('test-span-2', 0);
    const context2 = span2.spanContext();

    expect(context.traceId).not.toBe(context2.traceId);
    expect(context.spanId).not.toBe(context2.spanId);
  });
