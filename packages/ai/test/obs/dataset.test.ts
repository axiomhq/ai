...
describe('dataset commands', () => {
  ...
  it('renders dataset list in table and csv', async () => {
    ...
    expect(csvResult.stdout).toContain('name,description,created_at,modified_at');
  ...
  it('maps dataset metadata aliases across output formats', async () => {
    ...
    expect(tableResult.stdout).toContain('2026-01-07T00:00:00Z');

    const csvResult = await runCli(['datasets', 'get', 'vercel', '--format', 'csv'], { env });
    expect(csvResult.exitCode).toBe(0);
    expect(csvResult.stdout).toContain('name,description,created_at,modified_at');
    expect(csvResult.stdout).toContain(
      'vercel,Vercel dataset,2026-01-06T00:00:00Z,2026-01-07T00:00:00Z',
    );
  ...
  it('preserves explicit null metadata values when alias fields are present', async () => {
