...
import {
  ...
  resolveLogsDataset,
  resolveTraceDataset,
} from '../../src/cli/commands/servicesCommon';
...
describe('cli dataset schema cache', () => {
  const originalCacheTtl = process.env.AXIOM_CLI_SCHEMA_CACHE_TTL_MS;

  beforeEach(() => {
    process.env.AXIOM_CLI_SCHEMA_CACHE_TTL_MS = '60000';
    clearCliSchemaCache();
  });
  ...
  afterEach(() => {
    ...
    if (originalCacheTtl === undefined) {
      delete process.env.AXIOM_CLI_SCHEMA_CACHE_TTL_MS;
      return;
    }

    process.env.AXIOM_CLI_SCHEMA_CACHE_TTL_MS = originalCacheTtl;
  });
